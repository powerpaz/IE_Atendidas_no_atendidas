...
// ==================== ESTADO OFICIAL (COMPARTIDO) ====================
// Si existe data/state_official.json con useOfficial=true, ese estado se aplica para TODOS los usuarios.
// Esto resuelve el problema: “en mi PC veo 44.88% y en otras ven 26.15%”.
const OFFICIAL_STATE_URL = 'data/state_official.json';
const STATE_SOURCE_KEY = 'dece_state_source'; // 'official' | 'local'

let _officialStateMeta = null; // { stateVersion, publishedAt, publishedBy }

function getAppVersion(){
  return (window.APP_VERSION || 'dev').toString();
}
function getStateVersion(){
  return (window.STATE_VERSION || getAppVersion()).toString();
}

async function tryLoadOfficialState(){
  // Cache-buster para evitar que el CDN entregue un state_official.json viejo.
  const v = encodeURIComponent(getStateVersion());
  const url = `${OFFICIAL_STATE_URL}?v=${v}`;
  try {
    const res = await fetch(url, { cache: 'no-store' });
    if (!res.ok) return { ok: false, reason: `HTTP ${res.status}` };
    const text = await res.text();
    const json = JSON.parse(text);
    if (!json || json.useOfficial !== true) return { ok: false, reason: 'useOfficial=false' };
    if (!Array.isArray(json.editableBuffers) || !Array.isArray(json.customBuffers)) return { ok: false, reason: 'Estructura inválida' };

    // Guardar meta para mostrar en UI
    _officialStateMeta = {
    ...
    // IMPORTANTÍSIMO: se escribe en STORAGE_KEY para que el pipeline existente (drawBuffersEditable/custom) lo use.
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({
        editableBuffers: json.editableBuffers,
        customBuffers: json.customBuffers,
        timestamp: json.publishedAt || new Date().toISOString(),
        version: json.version || 'official'
      }));
      localStorage.setItem(STATE_SOURCE_KEY, 'official');
    } catch(_) {}
...
function publishOfficialStateDownload(){
  // Descarga un state_official.json listo para reemplazar en el repo.
  const now = new Date();
  ...
  const payload = {
    ...
    publishedAt,
    publishedBy: '',
    note: 'Reemplaza /data/state_official.json en el repo por este archivo y haz commit/push. Eso hará que TODOS vean el mismo mapa/exportación.',
    version: '6.2',
    editableBuffers: [],
    customBuffers: []
  };
  ...
  customBuffers.forEach(buffer => {
    const pos = buffer.circle.getLatLng();
    payload.customBuffers.push({
      id: buffer.id,
      lat: pos.lat,
      lng: pos.lng,
      name: buffer.name || buffer.orphanAlias || buffer.id,
  ...
  a.href = url;
  a.download = 'state_official.json';
  a.click();
  URL.revokeObjectURL(url);

  showNotification('✅ Descargado state_official.json. Súbelo al repo en /data/state_official.json (commit/push) para que todos vean el mismo resultado.', 'success');
}

function restoreFromOfficial(){
  // Limpia cambios locales y fuerza recarga: al iniciar, la app intentará cargar el state oficial del repo.
  try { localStorage.removeItem(STORAGE_KEY); } catch(_) {}
  try { localStorage.setItem(STATE_SOURCE_KEY, 'official'); } catch(_) {}
  showNotification('🔄 Restaurando desde estado oficial...', 'info');
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 1) Mapa + controles
  // 2) Intentar cargar ESTADO OFICIAL del repo (si existe)
  // 3) Cargar CSV y construir capas/buffers (que ya leerán STORAGE_KEY)
  (async () => {
    initMap();
    setupControls();
    setupEditControls();

    const r = await tryLoadOfficialState();
    if (r.ok) {
      console.log('✅ Estado oficial aplicado (repo)');
    } else {
      console.log('ℹ️ Estado oficial no aplicado:', r.reason);
      // Si no hay oficial, se mantiene el estado local existente (si lo hay).
      try { localStorage.setItem(STATE_SOURCE_KEY, 'local'); } catch(_) {}
    }
...
function loadCSV() {
  ...
  setText("Cargando CSV...", `DECE_CRUCE_X_Y_NUC_SAT.csv (v:${v})`);
  
  console.log("[LOAD] Iniciando fetch...");
  // Cache-buster: evita que otros usuarios vean un CSV viejo por cache del navegador/CDN.
  // GitHub Pages suele cachear agresivo archivos estáticos (sobre todo si el nombre no cambia).
  fetch(`DECE_CRUCE_X_Y_NUC_SAT.csv?v=${encodeURIComponent(v)}`, { cache: "no-store" })
    .then(res => { 
...
function setupControls() {
  // Publicar/Restaurar estado oficial (soluciona diferencias entre PCs)
  document.getElementById('btnPublishOfficial')?.addEventListener('click', () => {
    publishOfficialStateDownload();
  });
  document.getElementById('btnRestoreOfficial')?.addEventListener('click', () => {
    if (!confirm('Esto borrará tus cambios locales y recargará usando el estado oficial del repo. ¿Continuar?')) return;
    restoreFromOfficial();
  });
