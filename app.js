...
// Cantonal debe quedar como "contexto" (abajo) y provincias como borde (arriba)
map.getPane("paneCantonal").style.zIndex = 405;

// Puntos IE siempre arriba
map.createPane("paneIE");
map.getPane("paneIE").style.zIndex = 430;
  ...
  checkboxEl.addEventListener("change", async () => {
    if (checkboxEl.checked) {
      if (!layers[layerKey]) {
        ...
        // Orden recomendado: cantonal abajo, provincias como borde, IE arriba
        if (layerKey === "cantonal" && layers[layerKey].bringToBack) layers[layerKey].bringToBack();
        else if (layers[layerKey].bringToFront) layers[layerKey].bringToFront();

        if (layers.cantonal && layers.cantonal.bringToBack) layers.cantonal.bringToBack();
        if (layers.provincias && layers.provincias.bringToFront) layers.provincias.bringToFront();
        if (layers.ie && layers.ie.bringToFront) layers.ie.bringToFront();
      }
    } else {
  ...
  try {
    ...
    layers.cantonal = L.geoJSON(data, {
      ...
      style: (feature) => {
        ...
        return {
          // Estilo coroplético visible (sin borde negro)
          weight: 0.6,
          color: "#ffffff",
          opacity: 0.9,
    ...
    // Cantonal abajo, puntos IE arriba
    if (layers.cantonal.bringToBack) layers.cantonal.bringToBack();
    if (layers.provincias && layers.provincias.bringToFront) layers.provincias.bringToFront();
    if (layers.ie && layers.ie.bringToFront) layers.ie.bringToFront();
    // No sobreescribir mensajes más útiles (ej. conteos IE)
    if (($status?.textContent || "").trim() === "") setStatus("Listo.");
  } catch (e) {
...
(async () => {
  try {
    ...
    // Cantonal: si está marcado por defecto, lo cargamos de una vez (para que el coroplético se vea)
    if (ui.lyrCantonal?.checked) {
      await loadCantonalLazy();
    }

    // Capa IE: primero intenta CSV completo (recomendado). Si no existe, usa el GeoJSON light.
    // Importante: si por filtros quedan 0 puntos, mostramos la capa "excluidos" en gris para que el usuario
    // vea representación espacial (sin romper las reglas), y el status lo explica.
    const ieFromCsv = await loadIEFromCSV();
    const ieDataFinal = ieFromCsv || await loadGeoJSON("data/IE_Priorizacion_light.geojson");

    function iePasaFiltros(p) {
    ...
    function iePopupProps(p) {
      const nbiNum = toNum(p["% NBI_Cantón"] ?? p["% NBI_Cantón**"] ?? p.NBI);
      return {
        AMIE: p.AMIE ?? "",
        "IE": p.NOM_INSTITUCION_EDUCATIVA ?? p.IE ?? "",
        "Cantón": p.DPA_DESCAN ?? p.CANTON ?? "",
        "Provincia": p.DPA_DESPRO ?? p.PROVINCIA ?? p.DPA_DESPRO2 ?? "",
        "% NBI_Cantón": Number.isFinite(nbiNum) ? nbiNum.toFixed(3) : "Sin dato",
        ...
        "Mantenimiento C_I": (p["Mantenimiento C_I"] ?? p.Mantenimiento_C_I ?? "")
      };
    }

    const ieFiltrada = L.geoJSON(ieDataFinal, {
      pane: "paneIE",
      filter: (f) => iePasaFiltros(f.properties || {}),
      pointToLayer: (feature, latlng) => {
        const p = feature.properties || {};
        const nbi = toNum(p["% NBI_Cantón"] ?? p["% NBI_Cantón**"] ?? p.NBI);
        const casos = toNum(p["Total Casos"] ?? p.Total_Casos ?? p.TotalCasos);
        return L.circleMarker(latlng, {
          radius: Math.max(6, radioPorCasos(casos)),
          fillColor: colorPorNBI(nbi),
          fillOpacity: 0.9,
          color: "#0b1220",
          weight: 1,
      ...
      onEachFeature: (feature, layer) => {
        const p = feature.properties || {};
        layer.bindPopup(popupFromProps(iePopupProps(p), ["AMIE", "IE", "Cantón", "Provincia", "% NBI_Cantón", "Total Casos", "Estudiantes otras nacionalidades", "Sostenimiento"]));
      }
    });

    const ieExcluida = L.geoJSON(ieDataFinal, {
      pane: "paneIE",
      filter: (f) => !iePasaFiltros(f.properties || {}),
      pointToLayer: (_feature, latlng) => {
        return L.circleMarker(latlng, {
          ...
          color: "#111827",
          weight: 1,
          className: "ie-point-excluded"
        });
      },
      onEachFeature: (feature, layer) => {
        const p = feature.properties || {};
        layer.bindPopup(
          `<div style="font-weight:700;margin-bottom:6px;">IE (Excluida por filtros)</div>` +
          popupFromProps(iePopupProps(p), ["AMIE", "IE", "Cantón", "Provincia", "% NBI_Cantón", "Total Casos", "Estudiantes otras nacionalidades", "Sostenimiento", "Mantenimiento C_I"])
        );
      }
    });

    const nOk = ieFiltrada.getLayers().length;
    const nEx = ieExcluida.getLayers().length;

    // Grupo único para el checkbox "IE Priorizacion"
    layers.ie = L.layerGroup();
    if (nOk > 0) {
      layers.ie.addLayer(ieFiltrada);
      setStatus(`IE: ${nOk} cumplen filtros. (Excluidas: ${nEx})`);
    } else {
      // Si no hay nada que cumpla, mostramos excluidas para que el mapa tenga representación
      layers.ie.addLayer(ieExcluida);
      setStatus(`IE: 0 cumplen filtros. Mostrando ${nEx} excluidas en gris para referencia.`);
    }

    if (ui.lyrIE) ui.lyrIE.checked = true; // forzamos encendido por defecto
    layers.ie.addTo(map);
    if (layers.ie.bringToFront) layers.ie.bringToFront();

    // Orden visual final (cantonal abajo, provincias como borde, IE arriba)
    if (layers.cantonal && layers.cantonal.bringToBack) layers.cantonal.bringToBack();
    if (layers.provincias && layers.provincias.bringToFront) layers.provincias.bringToFront();
    if (layers.ie && layers.ie.bringToFront) layers.ie.bringToFront();
