...
(() => {
  ...
  const GEOJSON_PATHS = {
    ...
    nbi: 'data/cantones_nbi_mayor_50.geojson',
    viol: 'data/total_casos_violencia.geojson',
    otras: 'data/total_estudiantes_otras_nacionalidades.geojson',
    ieNo: 'data/ie_fiscales_no_atendidas.geojson',
    serv: 'data/servicios_agua_luz.geojson'
  };
  ...
  function bubbleSize(sum) {
    // Ajuste visual: valores típicos 0–100; tamaño 24–78
    const s = clamp(Math.sqrt(Math.max(0, sum)) * 6 + 18, 24, 78);
    return Math.round(s);
  }
  ...
  bindToggle('tgNbi', 'nbi', async () => {
    const gj = await loadGeoJSON(GEOJSON_PATHS.nbi);
